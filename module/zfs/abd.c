...
#define	ABD_LRU_MINBLOCKSHIFT	PAGE_SHIFT
...
abd_lru_cpu_purge(abd_lru_cpu_t *lc)
{
	...
	return (evicted_bytes >> PAGE_SHIFT);
}
...
abd_lru_purge(uint64_t pages)
{
	...
	for (i = 0; i < max_ncpus; i++)
		total_pages += abd_lru_percpu[i].alc_size >> PAGE_SHIFT;
...
abd_alloc(size_t size, boolean_t is_metadata)
{
	...
	if (!zfs_abd_scatter_enabled || size <= PAGE_SIZE)
